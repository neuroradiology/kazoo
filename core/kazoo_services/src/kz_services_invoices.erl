...
annotate(Services, CurrentInvoices, [ProposedInvoice|ProposedInvoices], Invoices) ->
    BookkeeperId = kz_services_invoice:bookkeeper_id(ProposedInvoice),
    ProposedItems = kz_services_invoice:items(ProposedInvoice),
    case split_invoices(CurrentInvoices, BookkeeperId) of
...
create_invoices_foldl(Services) ->
    fun(BookkeeperId, PlansList, Invoices) ->
            PlansJObjs = [kz_services_plan:jobj(Plan)
                          || Plan <- PlansList
                         ],
            PlansJObj = kz_services_plans:merge(PlansJObjs, 'true'),
            [kz_services_invoice:create(Services, BookkeeperId, PlansJObj)
             |Invoices
            ]
    end.
...
split_invoices(Invoices, BookkeeperId) ->
    case lists:splitwith(fun(Invoice) ->
                                 kz_services_invoice:bookkeeper_id(Invoice) =:= BookkeeperId
                         end, Invoices)
    of
        {[], RemainingInvoices} ->
...
public_json(Thing) ->
    ...
    kz_json:from_list(
      [{kz_services_invoice:bookkeeper_id(Invoice)
