...
-record(transaction, {account_id :: kz_term:ne_binary()
                     ...
                     ,metadata = kz_json:new() :: kz_term:object()
                     ,audit = kz_json:new() :: kz_term:object()
...
maybe_set_bookkeeper(Account) ->
    case kz_transactions:bookkeeper(Account) of
        {'error', _} -> [];
        {'ok', Bookkeeper} ->
            [{fun set_bookkeeper_type/2
             ,kz_transactions:bookkeeper_type(Bookkeeper)
             }
            ,{fun set_bookkeeper_vendor_id/2
             ,kz_transactions:bookkeeper_vendor_id(Bookkeeper)
             }
            ]
    end.
