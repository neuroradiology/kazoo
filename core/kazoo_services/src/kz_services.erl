...
manual_quantities(#kz_services{manual_quantities='undefined'}=Services) ->
    Updates = manual_updates(Services),
    Quantities = kzd_services:manual_quantities(services_jobj(Services)),
    sum_quantities_updates(Quantities, Updates);
manual_quantities(#kz_services{manual_quantities=Quantities}) ->
    Quantities.
...
-spec set_manual_updates(services(), kz_services_quantities:billables()) -> services().
set_manual_updates(#kz_services{}=Services, Updates) ->
    Services#kz_services{manual_updates=Updates}.
...
add_audit_log_changes_type(Services, AuditLog) ->
    case {kz_json:get_ne_binary_value([<<"changes">>, <<"type">>], AuditLog) =:= 'undefined'
         ,kz_term:is_empty(cascade_updates(Services))
         ,kz_term:is_empty(account_updates(Services))
         }
    of
        {'false', _, _} -> AuditLog;
        {'true', 'false', 'false'} ->
            kz_json:set_value([<<"changes">>, <<"type">>], <<"multiple_updates">>, AuditLog);
        {'true', 'true', 'false'} ->
            kz_json:set_value([<<"changes">>, <<"type">>], <<"account_updates">>, AuditLog);
        {'true', 'false', 'true'} ->
            kz_json:set_value([<<"changes">>, <<"type">>], <<"cascade_updates">>, AuditLog);
        {'true', 'true', 'true'} ->
            kz_json:set_value([<<"changes">>, <<"type">>], <<"reconcile">>, AuditLog)
    end.
...
apps_foldl(_BookkeeperId, PlansList, Apps) ->
    PlansJObjs = [kz_services_plan:jobj(Plan)
                  || Plan <- PlansList
                 ],
    PlansJObj = kz_services_plans:merge(PlansJObjs, 'true'),
